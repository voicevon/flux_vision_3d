...
| **Dashboard `[1]`** | **工位与数据管理中枢 (Workspace Hub)**<br>`tools/workspace_hub/` | **1280x720 工位与数据总控台（左右两栏 + 四页签）**：工位工作空间/数据容器管理、图片卡片网格墙（双击放大）、Tag 白名单矩阵、健康体检大屏；直接执行 `python -m tools.workspace_hub` 即可启动 |
...
工位与数据管理中枢 (`tools/workspace_hub/`) 采用 1280×720 深色工业科技风**左右两栏**设计，定位为**“工位管理与数据工作空间”**，是连接现场数据采集、离线解算与生产部署的核心中枢。

#### 1. 架构解耦与核心职责
- **场景即工作空间 (Workspace)**：场景是数据的容器（包含样本图像 `raw_images/`、元数据 `scene_meta.yaml`、平差地图 `tags_map.yaml` 与质检报告 `reports/`），与具体的相机传感器和标定算法解耦；
- **专职采图工具委托**：Workspace Hub 自身不再运行 USB 相机取流循环，避免占用相机硬件句柄。采图统一由**主仪表盘 (Dashboard)** 入口唤起专职采图向导 `capture_wizard.py`，采图结束后中枢自动平滑重载新图像并刷新状态元数据；
- **页签区沙盒胶囊**：标定相册页签右上角胶囊直观反馈当前工位的沙盒隔离状态（`SANDBOX: ★ 生产运行基准` / `SANDBOX: 已平差 X.XXpx` / `SANDBOX: 草稿沙盒`），与页面帧数信息合并展示，彻底脱离对相机连接的硬依赖。
...
- **生产基准 (Production)**：被机械臂与跟踪程序读取的现场真值地图（`config/tags_map.yaml`），卡片右侧授予金牌徽章 `★ 生产运行`；平差精度达标后点击卡片上的 `生效生产` 按钮即可一键安全发布。

#### 2. 左右两栏布局与四页签动态区

| 区域 | 布局结构 | 说明 |
| :--- | :--- | :--- |
| **左栏 (0~340)** | 工位卡片列表（6 张）+ 全局操作按钮 | 切换页签时**保持稳定不动** |
| **右栏 (340~1280)** | 随页签动态切换的四片内容区 | 点击顶部页签胶囊切换 |
| **页签 1 ⊞ 标定相册** | 4 列 × 3 行图片卡片网格墙（卡片 218×182） | 单击选中 / 双击放大 / 滚轮翻页；右上角为沙盒状态胶囊 |
| **页签 2 ⚑ Tag白名单** | 白名单生效徽章 + 0~29 号放行矩阵 | 支持 `[刷新]` / `[编辑]` 按钮 |
| **页签 3 ▤ 体检报告** | 几何健康大屏 + 质检放行徽章 + Tag 覆盖矩阵 | 由原"纯净看板"演进而来 |
| **页签 4 ▣ 生产相册** | 生产基准工位相册（只读检视） | 与标定相册同款卡片网格墙 |

> 全量交互均为**鼠标操作**（已移除所有键盘快捷键）；双击卡片进入 940px 全宽大图沉浸视口，再次双击画面或点击 `返回网格` 回到卡片网格墙。
...
│   │   └── hub_renderer.py        #      1280x720 两栏四页签科技看板渲染引擎
