...
| **Dashboard `[1]`** | **工位与数据管理中枢 (Workspace Hub)**<br>`tools/workspace_hub/` | **960x720 紧凑工业总控台（左右两栏 + 动态四页签，严格上对齐）**：工位沙盒管理、内嵌运维操作条、图片卡片网格墙（双击全宽放大）、Tag 白名单放行矩阵、几何健康与数据一致性自愈 Dashboard；执行 `python -m tools.workspace_hub` 即可启动 |
...
工位与数据管理中枢 (`tools/workspace_hub/`) 采用 960×720 精致深色工业科技风**左右两栏**设计，定位为**“工位管理与数据工作空间”**，是连接现场数据采集、离线解算与生产部署的核心中枢。

#### 1. 架构解耦与核心职责
- **工位即工作空间 (Workspace)**：包含样本图像 `raw_images/`、元数据 `workspace_meta.yaml`、平差地图 `tags_map.yaml` 与质检报告 `reports/`；
- **全鼠标化与无快捷键**：所有交互均由精准鼠标点击驱动，界面无冗余按键说明，轻量纯净；
- **操作内嵌收敛与去“发布”化**：彻底移除左侧工位右键菜单，所有运维动作（重命名、打开文件夹、修改备注、更新元数据、克隆、删除）全部收拢在 Dashboard 卡片 #1 中；
- **数据一致性核验与自动自愈**：Dashboard 实时对比元数据记录与物理磁盘照片真值，提供 `[更新元数据]` 按钮一键纠偏，杜绝数据矛盾。

#### 2. 左右两栏布局与四页签动态区

| 区域 | 布局结构 | 说明 |
| :--- | :--- | :--- |
| **左栏 (0~340px)** | 工位卡片列表（6 张）+ `+ 新建 Workspace` | 切换页签时**保持稳定不动** |
| **右栏 (340~960px)** | 随页签动态切换的四片内容区 | 点击顶部 Header 胶囊一键直达 |
| **Tab 0: Dashboard** | 工位运维操作条 + 几何健康大屏 + 数据一致性诊断 | 包含 `[更新元数据]`、`[克隆工位]`、`[删除]` 等核心按钮 |
| **Tab 1: Tag白名单** | 白名单生效状态 + 0~29 号放行矩阵 | 支持 `[刷新]` / `[编辑]` 快速配置 |
| **Tab 2: 标定相册** | 3 列 × 3 行大卡片网格墙（每页 9 张） | 单击选中 / 双击全宽大图 / 滚轮翻页；纯净无红字空状态 |
| **Tab 3: ★ 生产相册** | 生产基准工位相册（只读检视） | 与标定相册同款网格架构 |

> 窗口纵向拉伸时实行严格的 `pad_y = 0` **上对齐**策略，绝无大黑边下坠；底层采用跨平台原生对话框 `src/utils/dialog_utils.py`，彻底拔除 Tkinter 依赖。
