...
graph TD
    subgraph 离线超精建图五步流水线
        A["工序 1: 安装标靶<br>Tag 0: SCARA 原点<br>Tag 1~29: 静止机架"] --> B["工序 2: 采图向导拍摄 10~20 张<br>(capture_wizard.py)<br>1080P @ 8fps 极速取景连拍"]
...
### 5.4 两阶段平差与 MAD 鲁棒清洗
1. **阶段一（Cauchy 鲁棒平差）**：采用 Cauchy 鲁棒损失函数降低离群点对整体几何结构的拉偏；
2. **MAD 统计清洗**：基于中位数绝对偏差（MAD）动态计算重投影残差门限，清洗大残差观测；
3. **阶段二（微容差精平差）**：设置 `ftol=1e-9`, `gtol=1e-9` 进行极致深层收敛，实测真实照片 RMSE 压降至 4.14px。

### 5.5 平面 PnP 180° 翻转二义性消除 (`expected_z_cam` 法向先验)

**病因分析**：`solve_single_tag_pnp` 采用 `SOLVEPNP_IPPE_SQUARE`，平面靶 PnP 天然存在双解二义性——两解沿靶面内一轴相差约 180°、法向翻转。斜视 (约 45°) 时两解重投影误差之差缩至像素噪声量级，纯误差择优会间歇性选中翻转解，表现为在线识别蓝棱柱与地图理论绿棱柱相差 180° (Z 轴反向)。

**修复方案**：`src/calibration/offline_engine.py` 与 `tools/calibration/tag_map_builder.py` 的 `solve_single_tag_pnp` 统一新增 `expected_z_cam` 法向先验参数——先剔除法向与先验反向的翻转解，再按误差择优；先验下无同向合格解时拒绝输出 (防错优先)；建图版同步补齐深度非法解过滤。

**先验接入点** (6 处调用点统一接入)：
- 地图内 Tag：用 BA 理论位姿法向作为先验；
- 地图外 Tag (动态目标)：退用"标靶朝向天空"先验（即相机系下 $Z$ 分量为负方向）；
- tracker/studio 渲染与目标测距全部接入。

**验证**：合成数据参数扫描（距离 500~1200mm × 噪声 0.3~1.0px × 边长偏差 0~2%，5400 样本）：无先验翻转率最高 23%，带先验后 **0 翻转**。

### 5.6 角点几何保真度与 CONTOUR 轮廓拟合攻坚
...
| 工具名称 | 物理路径 | 定位与核心功能 |
| :--- | :--- | :--- |
| **工位与数据管理中枢** | `tools/workspace_hub/app.py` | 工序 0：采样工位与批次分组管理、左右两栏 + 右栏四页签动态区、黄金闭环 SOP (新建工位→采图→一键生效生产) |
