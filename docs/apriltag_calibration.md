...
### 5.8 独立的离线标定精度体检与 Leave-One-Out 盲测批量验证 (`tag_offline_verifier.py`)
在五步标定流水线中，BA 平差生成的标靶地图（`tags_map.yaml`）直接决定了下游生产环境的单目自定位与 3D 抓取精度。为了在不接入物理相机的情况下对已采集样本进行独立且严格的精度体检，系统引入了专门的离线盲测验证引擎：
- **核心定位与职责分工**：
  - **人工审核画板 (工序 4)**：清洗 BA 的**输入**（剔除模糊、倾角过大、严重遮挡的坏观测，保证 BA 求解不被污染）；
  - **离线精度体检 (工序 P / 25)**：严格验证 BA 的**输出**（透帧检测每个 Tag 空间坐标是否准确，系统性识别哪个 Tag 或哪一帧定位漂移）。
- **Leave-One-Out (LOO) 留一盲测数学机制**：
  - 针对采图集合中的每一帧图像，遍历该帧内可见的每一个已知标靶 $T_{blind}$；
  - **盲测隔离**：将 $T_{blind}$ 从该帧解算集合中物理扣除；
  - **超定 RANSAC PnP 姿态反推**：由该帧剩余的可见标靶（Solver Tags，$\ge 2$ 个标靶对应 8 个以上点对），结合已建立的空间地图执行带 RANSAC 离群值剔除的位姿解算，精确反推相机的 6DoF 外参；
  - **前向盲测投影与残差量化**：利用反推的相机位姿将 $T_{blind}$ 的 3D 世界角点投影回该帧像平面，与物理实测角点比对，计算 4 角点平均欧氏像元误差（$px$）与基于深度推算的物理空间偏差（$mm$）。
- **双模数据源与智能闭环诊断**：
  - **已审核清单模式 (`--source manifest`，默认/极速)**：直接加载 `tag_observations.yaml` 中已审核的有效样本，22 帧仅耗时 ~6 秒。排除已知坏样本干扰，纯粹客观地度量 BA 地图全局内聚几何精度；
  - **原图重检测模式 (`--source images`)**：直接面向磁盘 1080P 原始图片重新执行双路互补融合检测，借助 RANSAC 鲁棒解算，端到端检验原图检测鲁棒性，并能反向自动发现人工审核遗漏的瑕疵帧。
- **体检成果与可视化输出**：
  - 自动输出 Markdown 综合体检报告（`data/tag_calibration_verification/offline_verification_report.md`），量化中位像元误差、中位空间偏差、Per-Tag 统计分布，并智能标出偏差嫌疑标靶与建议回审帧；
  - 逐帧输出渲染带有浅灰虚线实测框、彩色盲测反推框与 15 倍残差矢量的可视化体检图（`data/tag_calibration_images/visualized/*_loo_verify.png`）。
